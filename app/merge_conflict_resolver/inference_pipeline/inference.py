...
class MergeConflictResolver:
    ...
    def decode_special_tokens(self, tokens):
        ...
        for token in tokens:
            if token == self.NEWLINE_PLACEHOLDER:
                ...
                decoded.append(f"{self.SPACE_TOKEN}{token[len(self.SPACE_PLACEHOLDER):]}")
            else:
    ...
    def preprocess_merge_conflict(self, base_code, branch_a_code, branch_b_code):
        ...
        merged_tokens = self.git_merge_tokens(base_tokens, branch_a_tokens, branch_b_tokens)
        ...
        if len(input_ids) > self.MAX_CONFLICT_LENGTH:
            input_ids = input_ids[:self.MAX_CONFLICT_LENGTH-1] + [self.tokenizer.eos_token_id]
    ...
    def git_merge_tokens(self, base_tokens, a_tokens, b_tokens):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    result = subprocess.run(
                        [
                            'git', 'merge-file',
                            '-L', 'a', '-L', 'base', '-L', 'b',
                            a_path, base_path, b_path, '--diff3', '-p'
                        ],
                ...
                final_tokens = [self.tokenizer.bos_token] + decoded_tokens + [self.tokenizer.eos_token]
    ...
    def generate_resolution(self, preprocessed_input):
        ...
        with torch.no_grad():
            try:
                ...
                print(f"First few input tokens: {self.tokenizer.decode(preprocessed_input['input_ids'][0][:10])}")
                raise
    ...
    def resolve_conflict(self, base_code, branch_a_code, branch_b_code):
        ...
        preprocessed = self.preprocess_merge_conflict(base_code, branch_a_code, branch_b_code)

...
class PRPrioritizer:
    def __init__(self, model_path="./app/models/pr_priority_model.pkl"):
        ...
        self.author_association_columns = model_data.get(
            "author_association_columns", []
        )
        self.feature_means = model_data.get("feature_means", {})
        self.feature_stds = model_data.get("feature_stds", {})
        self.expected_feature_count = model_data.get(
            "expected_feature_count", 1548
        ...
        self.bert_model = RobertaModel.from_pretrained(
            "distilroberta-base", output_hidden_states=True, torchscript=True
        ).to(self.device)
    ...
    def extract_features(self, df, body_embeddings, title_embeddings):
        ...
        author_dummies = pd.get_dummies(
            df["author_association"].fillna("NONE"), prefix="author"
        )
        ...
        author_values = (
            author_dummies[self.author_association_columns].values
            if self.author_association_columns
            else pd.get_dummies(
                df["author_association"].fillna("NONE"), prefix="author"
            ).values
        )
        ...
        final_features = (
            np.vstack(final_features_list) if final_features_list else np.array([])
        )

        # Check if we have the right number of features and adjust if necessary
        feature_count = final_features.shape[1]
        logger.info(
            f"Feature matrix shape: {final_features.shape}, expected: {self.expected_feature_count}"
        )

        if feature_count != self.expected_feature_count:
            logger.warning(
                f"Feature count mismatch: got {feature_count}, expected {self.expected_feature_count}"
            )
        ...
        try:
            ...
            if "Feature shape mismatch" in str(e):
                ...
                if match:
                    ...
                    logger.info(
                        f"Updating feature count from error: expected={expected}, got={got}"
                    )
